...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if volume >= len(contents):
            self.contentsog = contents
        else:
            self.contentsog = ''
        self.volumeog = volume
        self.volume = volume
    ...
    def double(self):
        ...
        #    ** TWO **   LINES OF CODE.
        ################################################################
        f = self.append_string(self.contents)
        return f
    def shrink(self, new_volume):
        ...
        if new_volume < len(self.contents):
            for k in range(new_volume,len(self.contents)):
                cut = cut + self.contents[k]
            self.contents = ''
            if new_volume < len(self.contentsog):
                for k in range(new_volume):
                    self.contents = self.contents + self.contentsog[k]
    ...
    def double_then_shrink(self, new_volume):
        ...
        c = self.contentsog + self.contentsog
        if len(self.contentsog) * 2 > self.volume:
            self.contents = ''
            for k in range(self.volume):
                self.contents = self.contents + c[k]
        return len(a)+len(b)
    def reset(self):
        """
        What comes in:
        ...
        self.contents = self.contentsog
        self.volume = self.volumeog
    ...
    def steal(self, other_box):
        ...
        #    ** TWO **   LINES OF CODE.
        ################################################################
        t = self.append_string(other_box.contents)
        other_box.contents = t
